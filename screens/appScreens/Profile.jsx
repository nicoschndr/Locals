            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {currentUser.blockedUsers && user.blockedUsers && (
                    <View style={{marginTop: windowHeight * 0.03}}>
                        ...
                        {user.recentActivities && currentUser.recentActivities && user.recentActivities.length > 0 && (
                            <Text
                                style={[
                                    styles.text,
                                    styles.recent,
                        ...
                        >
                            {auth.currentUser.uid === uid && currentUser.recentActivities && currentUser.recentActivities.length > 0 && (
                                <View>
                                    {currentUser.recentActivities.map((recent) => (
                                        <View style={{flexDirection: 'row'}}>
                                            <View style={styles.recentItemIndicator}>
                                            </View>
                                            {recent.category === "user" && recent.action === "follow" && (
                                                <View style={{flexDirection: 'row'}}>
                                                    <Text>folgt jetzt </Text>
                                                    <TouchableOpacity onPress={() => {
                                                        navigation.navigate('Following', {ruid: recent.uid})
                                                    }}>
                                                        <Text style={{fontWeight: 'bold'}}>@{recent.username}</Text>
                                                    </TouchableOpacity>
                                                </View>
                                            )}
                                            {recent.category === "event" && recent.action === "create" && (
                                                <View style={{flexDirection: 'row'}}>
                                                    <Text>hat </Text>
                                                    <TouchableOpacity onPress={() => {
                                                        getEventByTitle(recent.title)
                                                    }}>
                                                        <Text style={{fontWeight: 'bold'}}>{recent.title} </Text>
                                                    </TouchableOpacity>
                                                    <Text>erstellt</Text>
                                                </View>
                                            )}
                                            {recent.category === "event" && recent.action === "participate" && (
                                                <View style={{flexDirection: 'row'}}>
                                                    <Text>nimmt an </Text>
                                                    <TouchableOpacity onPress={() => {
                                                        getEventByTitle(recent.title)
                            ...
                            {auth.currentUser.uid !== uid && user.recentActivities && user.recentActivities.length > 0 && (
                                <View>
